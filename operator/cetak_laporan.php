...
if ($b !== 'semua' && $cabang != 'semua') {
    $query = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE status >= '4' AND DATE_FORMAT(tgl_transaksi, '%Y-%m-%d') BETWEEN '$t-$b-01' AND '$t-$b-32' and id_cabang='$cabang'  ORDER BY status ASC, tgl_transaksi ASC");

}else if($cabang != 'semua' && $b =='semua'){
    $query = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE status >= '4' AND DATE_FORMAT(tgl_transaksi, '%Y') = '$t' and id_cabang='$cabang'  ORDER BY status ASC, tgl_transaksi ASC");
}else if($cabang == 'semua' && $b !='semua'){
