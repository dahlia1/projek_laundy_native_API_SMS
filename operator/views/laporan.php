...
if ($b !== 'semua' && $idc !='semua') {
    $query = mysqli_query($koneksi, "SELECT a.*,b.* FROM transaksi a,cabang b WHERE a.status >= '4' AND DATE_FORMAT(a.tgl_transaksi, '%Y-%m-%d') BETWEEN '$t-$b-01' AND '$t-$b-32' and a.id_cabang='$idc' and a.id_cabang=b.id_cabang ORDER BY a.status ASC, a.tgl_transaksi ASC");
}elseif ($idc == 'semua' && $b!='semua') {
    $query = mysqli_query($koneksi, "SELECT a.*,b.* FROM transaksi a,cabang b WHERE a.status >= '4' AND DATE_FORMAT(a.tgl_transaksi, '%Y-%m-%d') BETWEEN '$t-$b-01' AND '$t-$b-32'  and a.id_cabang=b.id_cabang ORDER BY a.status ASC, a.tgl_transaksi ASC");
}elseif ($idc != 'semua' && $b=='semua') {
    $query = mysqli_query($koneksi, "SELECT a.*,b.* FROM transaksi a,cabang b WHERE a.status >= '4' AND  a.id_cabang=b.id_cabang and a.id_cabang='$idc' ORDER BY a.status ASC, a.tgl_transaksi ASC");
}else {
    $query = mysqli_query($koneksi, "SELECT a.*,b.* FROM transaksi a,cabang b WHERE a.status >= '4' and a.id_cabang=b.id_cabang ORDER BY a.status ASC, a.tgl_transaksi ASC");
}
...
$idcabang=array();
$id=0;
$que=mysqli_query($koneksi, "SELECT * FROM cabang");
while($data=mysqli_fetch_array($que)){
    $cabang[$id]=$data['nama_cabang'];
    $idcabang[$id]=$data['id_cabang'];
    $id++;
}
                ...
                <div class="row">
                    ...
                    <div class="col p-1">
                        <select name="cabang" id="" class="form-control">
                            ...
                            <?php for ($a=0;$a <= strlen($cabang);$a++): ?>
                                <option value="<?= $idcabang[$a] ?>" <?php if ($idcabang[$a]==$idc): echo "selected"; ?>
                                    
                                <?php endif ?>><?php echo $cabang[$a] ?></option>
                            <?php endfor; ?>
                        </select>
                    </div>
                    <div class="col p-1">
        ...
        <div class="card">
            <div class="card-header with-border">
                ...
                <div class="d-inline float-right ml-auto text-right">
                    <a href="cetak_laporan.php?bulan=<?= $b ?>&tahun=<?= $t ?>&cabang=<?=$idc?>" class="btn btn-outline-primary py-1" target="_blank">
