...
$query = mysqli_query($koneksi, "SELECT * FROM transaksi WHERE status < '4' and id_cabang='$cabang' ORDER BY status ASC, tgl_transaksi ASC");
?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php elseif ($row['status'] == '2' && $row['logistik'] == 'Diantar'): ?>
                                        <!--    -->        
                                        <a href="#show" class="btn btn-secondary btn-xs py-1"  data-toggle="modal"  data-placement="top" data-id='<?php echo $row[kode_transaksi] ?>' title="Tandai Dikirim"><i class="fa fa-shipping-fast fa-2x">
                                    ...
                                    <?php elseif ($row['status'] == '3' || ($row['status'] == '2' && $row['logistik'] == 'Ambil Sendiri')): ?>
                                        <a href="sistem/update_status_pesanan.php?s=4&k=<?= base64_encode($row['kode_transaksi']) ?>&dr=<?php echo $row['id_pengantar'] ?>" class="btn btn-success btn-xs py-1" data-toggle="tooltip" data-placement="top"  title="Tandai Selesai"><i class="fa fa-check fa-2x"></i></a>
                                    <?php endif; ?>
